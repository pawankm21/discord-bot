...
def jokesDotCCDotcom():
	...
	'''
	randomLinkToGoToAPI = 'http://jokes.cc.com/feeds/random/' + \
		str(randint(1, 6779))
	JSONData = urllib.urlopen(randomLinkToGoToAPI).read()
	#parse data
	parsedJokeLink = JSONData[JSONData.index(
		'http'): JSONData.index('","queryString')].replace('\\', "")
...
def randomjokesDotcom():
	...
	jokeSectionText = soup.body.findAll('tr')[1].findAll(
		'td')[2].findAll('p')[1].get_text()  # magic
